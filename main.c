...
void erase_cross(int x, int y, int target_x, int target_y, int target_r,
                 int target_alive);
void buzzer_start(void);
void buzzer_stop(void);
void spawn_target(int* x, int* y, int* vx, int* vy, int r);
void joystick_update(int* cross_x, int* cross_y, int max_speed);
void target_update_diff(int old_x, int old_y, int* x, int* y, int* vx, int* vy,
                        int r);
int button_pressed(void);
void display_status(int score, int counter);
void game_start(int* cross_x, int* cross_y, int* target_x, int* target_y,
                int* target_vx, int* target_vy, int target_r);
...
int main(void) {
  ...
  while (1) {
    ...
    if (restart_pressed && !restart_prev) {
      ...
      game_start(&cross_x, &cross_y, &target_x, &target_y, &target_vx,
                 &target_vy, target_r);
    }
    ...
    if (game_running) {
      ...
      if (cross_x != o_cross_x || cross_y != o_cross_y) {
        erase_cross(o_cross_x, o_cross_y, target_x, target_y, target_r,
                    target_alive);
        draw_cross(cross_x, cross_y, 0x0000FFL);
        o_cross_x = cross_x;
        o_cross_y = cross_y;
      ...
      if (target_alive) {
        ...
        target_update_diff(old_target_x, old_target_y, &target_x, &target_y,
                           &target_vx, &target_vy, target_r);
      }
      ...
      if (pressed && !button_prev) {
        ...
        if (target_alive) {
          ...
          if (dx * dx + dy * dy <= target_r * target_r) {
            ...
            spawn_target(&target_x, &target_y, &target_vx, &target_vy,
                         target_r);
          }
        }
      }
...
void draw_circle(int cx, int cy, int r, uint32_t color) {
  for (int y = -r; y <= r; y++)
    for (int x = -r; x <= r; x++)
      if (x * x + y * y <= r * r) draw(cx + x, cy + y, 1, 1, color);
}
  ...
  *y += *vy;
  ...
  for (int py = min_y; py <= max_y; py++) {
    ...
    for (int px = min_x; px <= max_x; px++) {
      ...
      int old_in =
          (px - old_x) * (px - old_x) + (py - old_y) * (py - old_y) <= r * r;
