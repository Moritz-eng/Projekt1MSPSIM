...
static const char* keyboard_layout[] = {"ABCDEFGHIJ", "KLMNOPQRST", "UVWXYZ"}; //Tastaturlayout

char last_used_name[16] = ""; //Datenpersistenz global gespeichert wird bei OK gesetzt kann später wieder geladen werden
...
//Setzt: Cursor auf (0,0) und Eingabe leer und Flags zurück
...
//Zeichenabfrage Validiert: gültige Zeile/Spalte Sonderfälle in letzter Zeile
char keyboard_get_char(int x, int y) { 
  ...
  if (c == ' ') return 0;
  return c;
  //Gibt:Buchstaben zurück oder 0, wenn kein gültiger Key
}

//Hauptfunktion: Zeichnen der Tastatur
void keyboard_draw(void) {
  //1. Titel zeichnen
  setText(10, 5, "ENTER NAME:", COLOR_BLACK, COLOR_WHITE);
  //2. letzten Name Button
  int x = KEY_START_X;
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 10; col++) {
      ...
      if (row == 2) {
        if (col == 6) { //Unterscheidung zwischen Buchstaben, DEL und OK
...
//Cursor-Update (Rendering optimiert) alte Position → normal zeichnen neue Position → hervorgehoben zeichnen
//spart Rechenleistung wil kein Full-Redraw
...
void keyboard_update(KeyboardState* kb) {
  ...
  //Cursor neu zeichnen (nur wenn bewegt)
...
void keyboard_handle_input(KeyboardState* kb) {
  ...
  //Bewegung wird relativ gemessen
  ...
  if (pressed && !button_prev) {
    //Last Name lädt gespeicherten Namen
    if (kb->cursor_y == 3) {
      if (last_used_name[0] != '\0') {
      ...
      if (c && kb->input_pos < 8) {       //Maximallänge
        kb->input[kb->input_pos++] = c;  //Zeichen anhängen
        kb->input[kb->input_pos] = '\0';
        kb->input_changed = 1;
      }
    }
